...
public interface ProductRepository extends JpaRepository<Product, Long> {
	...
	@Query(value = "SELECT * FROM products WHERE status='1' and quantity>0 and hide=false and DATEDIFF(end_date,CURDATE())>0", nativeQuery = true)
	List<Product> findAllClient();

	@Query(value = "SELECT * FROM products WHERE product_id=?1", nativeQuery = true)
	Product findbyid2(Long id);

	@Query(value = "SELECT * FROM products WHERE status='1' and DATEDIFF(end_date,CURDATE())<7", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM products WHERE status='1' and quantity>0 and hide=false and DATEDIFF(end_date,CURDATE())>0 and category_id = ?", nativeQuery = true)
	...
	// List product new
	@Query(value = "SELECT * FROM products  WHERE status='1' and quantity>0 and hide=false and DATEDIFF(end_date,CURDATE())>0 ORDER BY entered_date DESC limit 20;", nativeQuery = true)
	public List<Product> listProductNew20();
	
	// Search Product
	@Query(value = "SELECT * FROM products WHERE status='1' and quantity>0 and hide=false and DATEDIFF(end_date,CURDATE())>0 and product_name LIKE %?1%" , nativeQuery = true)
