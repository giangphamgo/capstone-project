...
public class CartController extends CommomController {
	...
	public String add(@RequestParam("productId") Long productId,@RequestParam("quantity") String quantity, HttpServletRequest request, Model model) {
	...
	@Transactional
	public String checkedOut(Model model,@RequestParam("thanhpho") int city, Order order, HttpServletRequest request, User user)
		...
		if (StringUtils.equals(checkOut, "paypal")) {
			...
			for (CartItem cartItem : cartItems) {
				...
				int oderCheckCate=orderRepository.findOrderBycategoryId(cartItem.getProduct().getProductId());
				if (oderCheckCate==0){
					...
					Product product_quantity=productRepository.findbyid2(cartItem.getProduct().getProductId());
					product_quantity.setQuantity(product_quantity.getQuantity()-cartItem.getQuantity());
					productRepository.save(product_quantity);
					orderDetailRepository.save(orderDetail);
				}else {
					orderRepository.delete(order);
					return "redirect:/checkoutNull";
				}
			}
	...
	@GetMapping(value = "/checkoutNull")
	public String checkoutNull(Model model, User user) {
		commomDataService.commonData(model, user);

		return "web/checkoutNull";

	}
	@GetMapping(value = "/checkoutNullQuantity")
	public String checkoutNullQuantity(Model model, User user) {
		commomDataService.commonData(model, user);

		return "web/checkoutNullQuantity";
